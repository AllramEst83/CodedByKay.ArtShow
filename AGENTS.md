...
- **Data & Media Handling:**
  ...
  - Raw artwork files are stored in `assets/drawings/`.
  - Direct static access to `assets/drawings/*` is blocked via `netlify.toml`. Images and metadata are served dynamically through the `/.netlify/functions/image` serverless endpoint.
